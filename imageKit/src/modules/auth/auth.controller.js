...
const uploadAvatar = async (req, res) => {
  try {
    ...
    if (!file) {
               console.log("3 no file");

      apiError.badRequest(res, "no file uploaded pls send file with feild 'avatar' ")
    } else {
            console.log("4 before service");

      const result = await authService.avatarUpload(req.user.id, file)
            console.log("5 after service");
...
export { register, getMe, verifyEmail, login, refresh, logout, forgotPassword, resetPassword, uploadAvatar };